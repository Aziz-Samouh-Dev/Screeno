                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-slate-400">{client.email ?? client.telephone ?? client.ville ?? 'No contact info'}</p>
                            </div>
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
                        ...
                        <Button variant="destructive" className="rounded-xl" onClick={() => {
                            if (confirm('Delete this client? This cannot be undone.')) router.delete(`/clients/${client.uuid}`);
                        }}>
                ...
                <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
                    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                        <div className="flex items-center gap-2 mb-3">
                            ...
                            <p className="text-xs font-bold text-slate-400 uppercase tracking-wide">Invoiced</p>
                        </div>
                    ...
                    <div className="rounded-2xl border border-green-100 bg-green-50 p-5 shadow-sm">
                        <div className="flex items-center gap-2 mb-3">
                            ...
                            <p className="text-xs font-bold text-green-500 uppercase tracking-wide">Paid</p>
                        </div>
                    ...
                    <div className="rounded-2xl border border-purple-100 bg-purple-50 p-5 shadow-sm">
                        <div className="flex items-center gap-2 mb-3">
                            ...
                            <p className="text-xs font-bold text-purple-400 uppercase tracking-wide">Returned</p>
                        </div>
                        <p className="text-xl font-bold text-purple-800">{fmt(totalReturned)}</p>
                        <p className="text-xs text-purple-400 mt-1">{salesReturns.length} return{salesReturns.length !== 1 ? 's' : ''}</p>
                    </div>
                    ...
                    <div className="rounded-2xl bg-slate-900 p-5 shadow-xl">
                        ...
                        <p className="text-xs text-slate-500 mt-1">{balance <= 0 ? 'Fully settled' : 'Outstanding'}</p>
                    </div>
                </div>
                    ...
                    <div className="space-y-4">
                        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                            <h4 className="font-bold text-slate-800 mb-5 flex items-center gap-2">
                                <Building2 className="h-4 w-4 text-slate-400" /> Contact Info
                            </h4>
                            <div className="space-y-4">
                                {client.email && (
                                ...
                                <div className="flex items-start gap-3">
                                    ...
                                    <div>
                                        <p className="text-xs font-bold text-slate-400 uppercase tracking-tighter">Client Since</p>
                    ...
                    <div className="lg:col-span-2 rounded-3xl border border-slate-200 bg-white shadow-sm overflow-hidden">
                        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
                            <h3 className="flex items-center gap-2 font-bold text-slate-800">
                                <Clock className="h-4 w-4 text-slate-400" /> Recent History
                            </h3>
                            <div className="flex items-center gap-2">
                                        ...
                                        >
                                            ...
                                            <td className="px-5 py-3.5">
                                                <span className="flex items-center gap-1.5 text-xs font-medium">
                                                    {item.type === 'invoice' && <><FileText className="h-3.5 w-3.5 text-blue-500" /><span className="text-blue-700">Invoice</span></>}
                                                    {item.type === 'payment' && <><CreditCard className="h-3.5 w-3.5 text-green-500" /><span className="text-green-700">Payment</span></>}
                                                    {item.type === 'return'  && <><PackageX className="h-3.5 w-3.5 text-purple-500" /><span className="text-purple-700">Return</span></>}
                                                </span>
                                            ...
                                            <td className="px-5 py-3.5 text-right font-mono text-xs font-semibold">
                                                ...
                                                {item.type === 'invoice' && (item.remaining ?? 0) > 0 && (
                                                    <div className="text-xs text-red-400 font-normal">-{fmt(item.remaining!)} due</div>
                                                )}
                                            </td>
